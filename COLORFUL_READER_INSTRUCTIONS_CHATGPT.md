# Colorful Reader Agent — Instructions for ChatGPT
...
**Your output must be:** A single, pure bash script. No explanatory text. No code discussion. Just the executable bash script.
...
**Critical:** Output ONLY the bash script. Do not include explanatory text before or after the script.

---

## Output Length Considerations

**Be aware:** A complete bash script with 19 embedded HTML files can be 1,500+ lines. This may approach or exceed output limits in some contexts.

**If you encounter output limits:**
- You may split the output across multiple messages
- You may use a more compressed format for heredocs
- You may ask the user if they want a smaller library
- Use your judgment to handle this appropriately

The user will copy/paste your output into a `.sh` file, so ensure it's properly structured even if split across messages.

---

## Slug Requirements

**MANDATORY: All slugs must have `-gpt` suffix**
...
Examples:
"Foundations of Real Numbers" → "foundations-of-real-numbers-gpt"
"Gödel's Incompleteness" → "godel-s-incompleteness-gpt"
"Open Theism" → "open-theism-gpt"
```

**Path structure:**
```
LIBRARY_SLUG = {topic-slug}-gpt
SERIES_ID = s01, s02, s03 (zero-padded 2 digits)
SERIES_SLUG = {LIBRARY_SLUG}-{SERIES_ID}

Example for "Foundations of Real Numbers":
- Library: foundations-of-real-numbers-gpt
- Series 1: foundations-of-real-numbers-gpt-s01
- Series 2: foundations-of-real-numbers-gpt-s02
- Series 3: foundations-of-real-numbers-gpt-s03
```
...
- **Target branch:** Create new feature branch using `gpt/colorful-reader-agent-TIMESTAMP` pattern (this is the convention you've been using)
...
BRANCH="gpt/colorful-reader-agent-$(date +%Y%m%d-%H%M%S)"
...
LIBRARY_SLUG="foundations-of-real-numbers-gpt"
SERIES1_SLUG="foundations-of-real-numbers-gpt-s01"
SERIES2_SLUG="foundations-of-real-numbers-gpt-s02"
SERIES3_SLUG="foundations-of-real-numbers-gpt-s03"
...
### 4. Missing `-gpt` Suffix
❌ **WRONG:** `foundations-of-real-numbers`
✅ **CORRECT:** `foundations-of-real-numbers-gpt`
...
3. **A complete, executable bash script** ready to save as `.sh` and run

**If the output is too long:**
- You may split it across multiple responses
- Ensure each part is clearly marked (e.g., "Part 1/3", "Part 2/3")
- Ensure the parts concatenate correctly into a valid bash script
...
2. ✅ Feature branch created with `gpt/colorful-reader-agent-*` pattern
3. ✅ All slugs contain `-gpt` suffix
...
**Remember:** Output only the pure bash script. No explanations. Just the ready-to-execute bash script.
